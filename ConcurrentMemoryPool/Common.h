...
inline static void*& NEXT_OBJ(void* obj)//抢取对象头四个或者头八个字节（目的是存指向下一个对象的指针），void*的别名，本省是内存，只能我们自己取
{
...
{
private:
	void* _list = nullptr; // 给上缺省值，指向第一个对象
	size_t _size = 0;  // 记录有多少个对象，如果挂的对象太多，就要释放回central cache
	size_t _maxsize = 1;//最多能挂这么多对象

public:

	void Push(void* obj)//这个其实是push_front，把新插入的放在最前面
	{
		NEXT_OBJ(obj) = _list;
		_list = obj;
		++_size;
...
//Span是一个跨度，既可以分配内存出去，也负责将内存回收回来到PageCache合并
...
struct Span
{
	...
	void* _list = nullptr;//链接对象的自由链表，后面有对象就不为空，指向第一个未使用的对象
